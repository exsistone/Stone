...
let userID = '4c85d9e8-0168-4dde-abfc-4c6417cd3f30';
let kvUUID;
...
let ipLocal = [
	'visa.cn:443#github.com/exsistone',
	'icook.hk#http://exsi.ltd:5000:)',
	'time.is#github.com/exsistone'
];
let noTLS = 'false';
let sl = 5;
...
function getConfigHtml(userID, host, remark, v2ray, clash) {
    ...
    <head>
      ...
      <meta name='description' content='This is a project to generate free vmess nodes. For more information, please subscribe youtube and follow GitHub https://github.com/exsistone ' />
      <style>
        body {
...
async function checkKVNamespaceBinding(env) {
	if (typeof env.exsistone === 'undefined') {
		return new Response('Error: exsistone KV_NAMESPACE is not bound.', {
			status: 400,
		})
	}
}
...
async function getKVData(env) {
	const value = await env.exsistone.get(MY_KV_UUID_KEY);
...
async function setKVData(request, env) {
	...
	try {
		await env.exsistone.put(MY_KV_UUID_KEY, value);

		// 读取存入的值，确认是否成功
		const storedValue = await env.exsistone.get(MY_KV_UUID_KEY);
